...
class HeatmapMSELoss(nn.Module):
    ...
    def forward(self, output, target, target_weight):
        ...
        n_views = output.size(1)
        num_joints = output.size(2)
        heatmaps_pred = output.reshape((batch_size * n_views, num_joints, -1)).split(1, 1)
        heatmaps_gt = target.reshape((batch_size * n_views, num_joints, -1)).split(1, 1)
        loss = 0
